...
;var _export=$export,_iterators={},_iobject=Object("z").propertyIsEnumerable(0)?Object:function(it){
return"String"==_cof(it)?it.split(""):Object(it)
},_toIobject=function(it){
return _iobject(_defined(it))
},min=Math.min,_toLength=function(it){
return it>0?min(_toInteger(it),9007199254740991):0
},max=Math.max,min$1=Math.min,_arrayIncludes=function(IS_INCLUDES){
...
},shared=_shared("keys"),_sharedKey=function(key){
return shared[key]||(shared[key]=_uid(key))
},arrayIndexOf=_arrayIncludes(!1),IE_PROTO=_sharedKey("IE_PROTO"),_objectKeysInternal=function(object,names){
...
}.set,_collection=function(NAME,wrapper,methods,common,IS_MAP,IS_WEAK){
...
},_collectionStrong,!0),_core.Map,_arrayIncludes(!0))
;_export(_export.P,"Array",{includes:function(el){
...
for(var element,spreadable,targetIndex=start,sourceIndex=0,mapFn=!!mapper&&_ctx(mapper,thisArg,3);sourceIndex<sourceLen;){
if(sourceIndex in source){
if(element=mapFn?mapFn(source[sourceIndex],sourceIndex,original):source[sourceIndex],
spreadable=!1,
_isObject(element)&&(spreadable=void 0!==(spreadable=element[IS_CONCAT_SPREADABLE])?!!spreadable:_isArray(element)),
spreadable&&depth>0)targetIndex=flattenIntoArray(target,original,element,_toLength(element.length),targetIndex,depth-1)-1;else{
if(targetIndex>=9007199254740991)throw TypeError()
;target[targetIndex]=element}targetIndex++}
sourceIndex++}return targetIndex
},SPECIES$2=_wks("species"),_arraySpeciesCreate=function(original,length){
return new(function(original){var C
...
void 0===C?Array:C}(original))(length)}
;_export(_export.P,"Array",{
flatMap:function(callbackfn){
var sourceLen,A,O=_toObject(this)
;return _aFunction(callbackfn),sourceLen=_toLength(O.length),
A=_arraySpeciesCreate(O,0),
_flattenIntoArray(A,O,O,sourceLen,0,1,callbackfn,arguments[1]),
A}}),_addToUnscopables("flatMap"),function(){
